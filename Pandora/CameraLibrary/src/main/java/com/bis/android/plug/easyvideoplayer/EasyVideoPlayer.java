...
//                    mRestartDrawable = AppCompatResources.getDrawable(context, restartDrawableResId);
                    mRestartDrawable = context.getResources().getDrawable(restartDrawableResId);
                }
                if (playDrawableResId != -1) {
//                    mPlayDrawable = AppCompatResources.getDrawable(context, playDrawableResId);
                    mPlayDrawable = context.getResources().getDrawable(playDrawableResId);
                }
                if (pauseDrawableResId != -1) {
//                    mPauseDrawable = AppCompatResources.getDrawable(context, pauseDrawableResId);
                    mPauseDrawable = context.getResources().getDrawable(pauseDrawableResId);
                }
...
//        setPauseDrawable(AppCompatResources.getDrawable(getContext(), res));
    ...
    private void setSourceInternal() throws IOException {
        ...
        } else if (mSource.getScheme() != null && ("file".equals(mSource.getScheme()) && mSource.getPath().contains("/android_assets/"))) {
...
//            mPlayer.pause();
    ...
    private void initImage() {
        ...
        mmr.setDataSource(getContext(), mSource);

        Bitmap bitmap = mmr.getFrameAtTime();//获取第一帧图片
        mFirstFrameImageView.setImageBitmap(bitmap);
        mmr.release();//释放资源
    }
    ...
    protected void onFinishInflate() {
        ...
        final LayoutParams imageViewLp = new LayoutParams(ViewGroup.LayoutParams.MATCH_PARENT,
                ViewGroup.LayoutParams.MATCH_PARENT);
        mFirstFrameImageView = new ImageView(getContext());
        mFirstFrameImageView.setVisibility(GONE);
        addView(mFirstFrameImageView, imageViewLp);
