var labelArr = [['实物','明星商品'],['视频','片花'],['数据','文档']];
var pageIndexArr = [0,0,0];
var divIdArr = ['spsw','sphx','sjbg'];
var timer;
var type = 0;
var userId = "";
var token = "";
...
function back() {
    ...
    setBehaviourAction(h5Code);
}
var slideHeight=0;

function loadUserInfo()
{
    setUserInfoAction(initUserInfo);
}
	...
	$('.scroll-container').each(function(index){
		vSwiper = $(this).swiper({
			...
			onTouchEnd:function(swiper){
					...
					{
						if($(".content-slide").find("ul li").size()==0){
							loadSpswData();
							return;
						}
						$(".more .tip").html("正在加载");
...
function buildGoodsItems(item)
{
    ...
    box += '<a href="javascript:void(0)" onclick="showDetail('+item.goodsKind+',' + type + ',\'' + item.goodsId + '\')">'
	...
	if(item.goodsKind == 0){
        if(item.surplusNum==0){
            box += '<div class="front_bg" onclick="showDetail('+item.goodsKind+',' 
                    +type+',\''+item.goodsId+'\')"><span><i>暂时无货</i></span></div>';
        }
    }else if(item.goodsKind == 1){
        if(item.auctionStatus>0){
            box += '<div class="front_bg" onclick="showDetail('+item.goodsKind+',' 
                    +type+',\''+item.goodsId+'\')"><span><i>竞拍结束</i></span></div>';
        }else if(item.auctionStatus<0)
        {
            box += '<div class="front_bg" onclick="showDetail('+item.goodsKind+',' 
                    +type+',\''+item.goodsId+'\')"><span><i>即将开始</i></span></div>';
        }
    }
    box+= '</li>';
    return box;
...
function loadSpswData(flag){
    ...
    var data = '{"userId":"'+userId+'","header":"00010301","data":"{type:'+type+',requestType:'+1+',searchInput:1,page:0'+',rows:10}"}';
...
//	console.log(321);
	$.ajax({
		...
		timeout: 10000,
		url:urlR,
		data:'{"userId":"'+userId+'","header":"00010301","data":"{type:'+type+',requestType:'+1+',searchInput:1,page:'+pageNum+',rows:10}"}'  ,
		type:"post",
