 ...
 * Description : <br>
 */
public class ChooseVideoPopupWindow {

    public static final int PHOTO_TAKE_REQUEST_CODE = 0, VIDEO_TAKE_REQUEST_CODE = 1,
    ...
    public ChooseVideoPopupWindow(Activity activity, OnShowViewListener listener) {
        this.activity = activity;
        this.mShowViewListener = listener;
    }

    public void show(View view) {
        showWindow = new SelectPicPopupWindow(activity, mListener, "拍摄", "从相册选择", "录制");
...
//                    startActivityForResult(intent, GALLERY_REQUEST_CODE);
                ...
                case R.id.btn_click_one:
                    MaterialCamera materialCameraToPhoto = new MaterialCamera(activity)
                            .saveDir(FileUtils.getCachePath(activity, "img"))
        ...
        switch (requestCode) {
            case PHOTO_TAKE_REQUEST_CODE:
                if (resultCode == Activity.RESULT_OK) {
                    mShowViewListener.chooseSuccess(PHOTO_TAKE_REQUEST_CODE,
                            false, data.getData().getPath());
                }
                break;
            case GALLERY_REQUEST_CODE:
                if (resultCode == Activity.RESULT_OK) {
                    MediaEntity entity = data.getParcelableExtra("MEDIA_ENTITY");
                    mShowViewListener.chooseSuccess(GALLERY_REQUEST_CODE,
                            entity.isVideo(), entity.getPath());
                }
                break;
    ...
    private void getVideoFromIntent(Intent data, boolean takeVideo) {
        ...
        if (ContentResolver.SCHEME_FILE.equals(selectedVideo.getScheme())) {
            ...
            if (mime_type == null || "".equals(mime_type)) {
                ToastUtil.showShort(activity, "视频拍摄失败,请重新拍摄!");
                return;
            }
        ...
        mShowViewListener.chooseSuccess(VIDEO_TAKE_REQUEST_CODE,
                true, filePath);
    }

    public interface OnShowViewListener{
        void chooseSuccess(int type, boolean isVideo, String path);
    }
}
