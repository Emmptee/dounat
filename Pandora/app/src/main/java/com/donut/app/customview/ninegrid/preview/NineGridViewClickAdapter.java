...
import com.donut.app.customview.ninegrid.NineGridViewAdapter;
...
public class NineGridViewClickAdapter extends NineGridViewAdapter {
    ...
    protected void onImageItemClick(Context context, NineGridView nineGridView, int index, List<ImageInfo> imageInfo) {
        ...
        if (imageInfo.size() > nineGridView.getMaxSize()
                && index == nineGridView.getMaxSize() - 1) {
            //点击more
            Intent intent = new Intent(context, SpinOffBoonsDetailActivity.class);
            Bundle bundle = new Bundle();
            bundle.putString(SpinOffBoonsDetailActivity.B02_ID, imageInfo.get(index).getB02Id());
