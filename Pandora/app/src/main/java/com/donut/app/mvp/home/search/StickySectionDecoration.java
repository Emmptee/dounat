...
public class StickySectionDecoration extends RecyclerView.ItemDecoration {
    ...
    private Paint mPaint, mBottomPaint;
    ...
    public StickySectionDecoration(Context context, GroupInfoCallback callback) {
        ...
        mTextOffsetX = context.getResources().getDimensionPixelOffset(R.dimen.header_textsize);
        mBottomHeight = context.getResources().getDimensionPixelOffset(R.dimen.bottom_height);
        ...
        mBottomPaint.setColor(0xFFECECEC);
    }
        ...
        for (int i = 0; i < childCount; i++) {
            ...
            if (mCallback != null) {
                ...
                if (groupinfo.isLastViewInGroup()) {
                    c.drawRect(left, view.getBottom(), right, view.getBottom() + mBottomHeight, mBottomPaint);
                }
    ...
    private void drawHeaderRect(Canvas c, GroupInfo groupinfo, int left, int top, int right, int bottom) {
        ...
        c.drawRect(left, bottom, right, bottom + 1, mBottomPaint);

        float titleX = left + mTextOffsetX;
        float titleY = bottom - 10 - mFontMetrics.descent;
        //绘制Title
