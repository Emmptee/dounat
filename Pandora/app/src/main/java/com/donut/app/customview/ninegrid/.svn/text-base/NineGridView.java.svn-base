...
public class NineGridView extends ViewGroup {
    ...
    private float gridSpacing = 7.5f;                    // 宫格间距，单位dp
    ...
    public NineGridView(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        gridSpacing = TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_DIP, gridSpacing, dm);
        ...
        gridSpacing = a.getDimension(R.styleable.NineGridView_ngv_gridSpacing, gridSpacing);
...
//                gridWidth = gridHeight = (totalWidth - gridSpacing * (columnCount - 1)) / columnCount;
                ...
                gridWidth = gridHeight = (int) ((totalWidth - gridSpacing * 2) / 3);
            }
            width = (int) (gridWidth * columnCount + gridSpacing * (columnCount - 1) + getPaddingLeft() + getPaddingRight());
            height = (int) (gridHeight * rowCount + gridSpacing * (rowCount - 1) + getPaddingTop() + getPaddingBottom());
        }
    ...
    protected void onLayout(boolean changed, int l, int t, int r, int b) {
        ...
        for (int i = 0; i < childrenCount; i++) {
            ...
            int left = (int) ((gridWidth + gridSpacing) * columnNum + getPaddingLeft());
            int top = (int) ((gridHeight + gridSpacing) * rowNum + getPaddingTop());
    ...
    public void setAdapter(@NonNull NineGridViewAdapter adapter) {
        ...
        for (int i = 0; i < imageCount; i++) {
            ImageView iv = getImageView(i);
            if (iv instanceof NineGridViewWrapper) {
                NineGridViewWrapper imageView = (NineGridViewWrapper) iv;
                String url = imageInfo.get(i).getBigImageUrl();
                if (url != null && url.length() > 0
                        && (url.endsWith("gif") || url.endsWith("GIF"))) {
                    imageView.setAGif(true);
                } else {
                    imageView.setAGif(false);
                }
            }
        }
        //修改最后一个条目，决定是否显示更多
