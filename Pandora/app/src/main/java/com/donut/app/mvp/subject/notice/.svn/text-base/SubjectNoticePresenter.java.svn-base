 ...
 * Description : <br>
 */
public class SubjectNoticePresenter extends SubjectNoticeContract.Presenter {

    private static final int SUBJECT_NOTICE_DETAIL_CODE = 1,
    ...
    public void loadData(boolean showLoadingView, String subjectId) {
        ...
        super.loadData(request, HeaderRequest.SUBJECT_NOTICE_DETAIL,
                SUBJECT_NOTICE_DETAIL_CODE, showLoadingView);
    }

    @Override
    public void onSuccess(String responseJson, String url, int actionId) {
        switch (actionId) {
            case SUBJECT_NOTICE_DETAIL_CODE:
    ...
    void saveBehaviour(String functionCode) {
        SaveBehaviourDataService.startAction(mView.getContext(),
                BehaviourEnum.SUBJECT_NOTICE_DETAIL.getCode() + functionCode);
    }

    void saveBehaviour(String functionCode, Object request, String header) {
        SaveBehaviourDataService.startAction(mView.getContext(),
                BehaviourEnum.SUBJECT_NOTICE_DETAIL.getCode() + functionCode, request, header);
    }
}
