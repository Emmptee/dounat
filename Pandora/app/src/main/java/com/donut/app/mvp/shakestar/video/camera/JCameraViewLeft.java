package com.donut.app.mvp.shakestar.video.camera;
...
import com.donut.app.mvp.shakestar.video.camera.listener.TypeListener;
import com.donut.app.mvp.shakestar.video.camera.state.CameraMachine;
...
public class JCameraViewLeft extends FrameLayout implements CameraInterface.CameraOpenOverCallback, SurfaceHolder
        .Callback, CameraView {
    private static final String TAG = "JCameraView";
    private RecordLeftActivity mRecordLeftActivity;
    //Camera状态机
...
//    private TextView mNextButton;
    private TextView mMidCameraButton;
    private TextView mClickCameraButton;
    ...
    public JCameraViewLeft(Context context) {
        this(context, null);
    }

    public JCameraViewLeft(Context context, AttributeSet attrs) {
        this(context, attrs, 0);
    }

    public JCameraViewLeft(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        if (context instanceof RecordLeftActivity) {
            mRecordLeftActivity = (RecordLeftActivity) context;
        }
        //get AttributeSet
        ...
        iconSrc = a.getResourceId(R.styleable.JCameraView_iconSrc, R.drawable.ic_turn_camera);
...
//        duration = RecordActivity.VideoDuration;       //没设置默认为5s
    ...
    private void initView() {
        ...
        View view = LayoutInflater.from(mContext).inflate(R.layout.camera_view_left, this);
        mFrameLayout = (FrameLayout) view.findViewById(R.id.fl_camera);
        mRelativeLayout = (RelativeLayout) view.findViewById(R.id.ll_surfaceview);
//        mPlayerView = (SurfaceView) view.findViewById(R.id.sv_left);
...
/*        mNextButton.setEnabled(false);
        mNextButton.setOnClickListener(new OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                KLog.e("点击了下一步");
                mRecordLeftActivity.finish();
                RotateOutputVideo();

//                RecordActivity.getInstance().finish();
//                mCaptureLayout.setBackgroundColor(getResources().getColor(R.color.red));
            }
        });*/

        mClickCameraButton.setOnClickListener(new OnClickListener() {
            @Override
            public void onClick(View v) {
                mCaptureButton.setClickState();
                mClickCameraButton.setVisibility(GONE);
                mPressCameraButton.setVisibility(VISIBLE);
                mMidCameraButton.setText("单击拍摄");
                mButtonText.setText("单击拍");
            }
        });

        mPressCameraButton.setOnClickListener(new OnClickListener() {
            @Override
            public void onClick(View v) {
                mCaptureButton.setPressState();
                mClickCameraButton.setVisibility(VISIBLE);
                mPressCameraButton.setVisibility(GONE);
                mMidCameraButton.setText("长按拍摄");
                mButtonText.setText("长按拍");
            }
        });
...
//                stopPlayLeftVideo();
    ...
    private void sendRecordStartBroadcast() {
        ...
        mRecordLeftActivity.sendBroadcast(intent);
        KLog.e("发送了录像开始的广播");
    }
...
//                startActivity(v);
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        widthSize = mVideoView.getMeasuredWidth();
    ...
    public void surfaceCreated(SurfaceHolder holder) {
        KLog.i("JCameraView SurfaceCreated");
        new Thread() {
            @Override
            public void run() {
                CameraInterface.getInstance().doOpenCamera(JCameraViewLeft.this);
            }
        }.start();
    }
...
//        mCaptureLayout.startTypeBtnAnimator();
    ...
    public void playVideo(Bitmap firstFrame, final String url) {
        videoUrl = url;
        JCameraViewLeft.this.firstFrame = firstFrame;
