...
public class ChannelSearch2Presenter extends ChannelSearch2Contract.Presenter {
    ...
    void onToCollect(SubjectListDetail subjectDetail) {
        ...
        super.loadData(request, HeaderRequest.SUBJECT_COLLECT, SUBJECT_COLLECT, false);
    }

    public void onToShare(final SubjectListDetail subjectDetail) {

        String url = RequestUrl.SUBJECT_SNAP_SHARE_URL + "header="
...
//                    + ",【小伙伴】已围观了" + subjectDetail.getBrowseTimes();
        String title = subjectDetail.getStarName() + " | " + subjectDetail.getName();
        String content = subjectDetail.getDescription();
...
//        });
        ...
        request.setContentId(subjectDetail.getB02Id());
        super.loadData(request, HeaderRequest.SHARE, SUBJECT_SHARE, false);
    }
}