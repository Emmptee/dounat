...
{
    jmethodID meth = (*m_env)->GetStaticMethodID(m_env, jcls, "onEnd", "()V");

__android_log_print(ANDROID_LOG_INFO, "2222222222222222222222222222222222222222222222", "%s", "ffmpeg_cleanup");
    int i, j;
    ...
    term_exit();
    ffmpeg_exited = 1;
    (*m_env)->CallStaticVoidMethod(m_env, jcls, meth);
}
                         ...
                         double sync_ipts)
{
  ...
  for (i = 0; i < nb_frames; i++) {
     ...
     */
    ost->frame_number++;
    (*m_env)->CallStaticVoidMethod(m_env, jcls, methodID,ost->frame_number);
...
int run(int argc, char **argv, JNIEnv *env, jclass obj)
{
    m_env = env;
    jcls = (*env)->NewGlobalRef(env,obj);
    methodID = (*env)->GetStaticMethodID(env, jcls, "onProgress", "(I)V");
...
void notifyMedia(char* filename, int duration, double fps){
    jmethodID notifyMedia = (*m_env)->GetStaticMethodID(m_env, jcls, "notifyMedia", "(IDLjava/lang/String;)V");
    jstring filePath = (*m_env)->NewStringUTF(m_env, filename);
    (*m_env)->CallStaticVoidMethod(m_env, jcls, notifyMedia,duration, fps, filePath);
}
